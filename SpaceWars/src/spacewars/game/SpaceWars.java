...
import spacewars.game.model.Mine;
...
public class SpaceWars extends Game {
	...
	public void update(GameTime gameTime) {
		...
		if (Keyboard.getState().isKeyPressed(Key.H)) {
			...
			Screen.getInstance().getViewport()
					.setCentralPosition(home.x, home.y);
		}
		...
		if (Mouse.getState().isButtonDragged(Button.LEFT)) {
			...
			final int x = Screen.getInstance().getViewport()
					.getOriginPosition().x
					+ dx;
			final int y = Screen.getInstance().getViewport()
					.getOriginPosition().y
					+ dy;
		...
		if (Mouse.getState().isButtonPressed(Button.LEFT)) {
			for (GameElement element : map.getMineralPlanets()) {
				final Vector origin = Screen.getInstance().getViewport()
						.getOriginVector();
